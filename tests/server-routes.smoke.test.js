...
describe("gallery API", () => {
  ...
  test("GET /api/gallery returns later deterministic batches when page is provided", async () => {
    const metClient = createMetApiClient({
      async fetchImpl(resource) {
        const url = String(resource);
        ...
        if (url.includes("/search?")) {
          return createJsonResponse({
            total: 60,
            objectIDs: Array.from({ length: 60 }, (_, index) => 600 - index)
          });
        }
    ...
    const response = await makeRequest("/api/gallery?page=2", galleryApp);
    const payload = JSON.parse(response._getData());

    expect(response.statusCode).toBe(200);
    expect(payload.results).toHaveLength(24);
    expect(payload.results[0].objectId).toBe(565);
    expect(payload.results.at(-1).objectId).toBe(588);
  });

  test("GET /api/gallery uses shuffle as a stable seed for gallery ordering", async () => {
    const metClient = createMetApiClient({
      async fetchImpl(resource) {
        const url = String(resource);
        ...
        if (url.includes("/search?")) {
          return createJsonResponse({
            total: 30,
            objectIDs: Array.from({ length: 30 }, (_, index) => 300 - index)
          });
        }
    ...
    const defaultResponse = await makeRequest("/api/gallery", galleryApp);
    const firstShuffleResponse = await makeRequest("/api/gallery?shuffle=alpha-seed", galleryApp);
    const secondShuffleResponse = await makeRequest("/api/gallery?shuffle=alpha-seed", galleryApp);
    const defaultIds = JSON.parse(defaultResponse._getData()).results.map((work) => work.objectId);
    const shuffledIds = JSON.parse(firstShuffleResponse._getData()).results.map((work) => work.objectId);

    expect(firstShuffleResponse.statusCode).toBe(200);
    expect(JSON.parse(secondShuffleResponse._getData()).results.map((work) => work.objectId)).toEqual(
      shuffledIds
    );
    expect(shuffledIds).not.toEqual(defaultIds);
  });
