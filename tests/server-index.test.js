...
import { createProductionArtctlApp } from "../server/index.js";
...
describe("server index startup", () => {
  ...
  test(
    "createProductionArtctlApp returns catalog readiness metadata for an initialized but empty CATALOG_DATABASE_PATH",
    async () => {
      const tempDir = createTrackedTempDir(path.join(os.tmpdir(), "artctl-server-index-"));
      const databasePath = path.join(tempDir, "catalog.sqlite");
  ...
  test("createProductionArtctlApp loads CATALOG_DATABASE_PATH from .env.local", async () => {
    const tempDir = createTrackedTempDir(path.join(os.tmpdir(), "artctl-server-index-"));
    const databasePath = path.join(tempDir, "catalog.sqlite");
